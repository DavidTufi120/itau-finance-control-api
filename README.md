...
- **Autenticação via api-key**: implementada com um `Filter` da API Servlet, retornando `401` em JSON estruturado quando a chave está ausente ou inválida.
...
|---|---|---|
| `GET /actuator/health` | Público | Retorna o status da aplicação (`UP` ou `DOWN`) |
| `GET /actuator/metrics` | Protegido por `api-key` | Lista as métricas disponíveis |
...
A aplicação também está publicada no **Railway** para demonstração técnica do funcionamento em ambiente de produção, incluindo banco MySQL, healthcheck e monitoramento.
...
> As rotas de negócio em `/v1/**` exigem o header `api-key: aXRhw7o=`.
> O endpoint `/actuator/metrics` também exige `api-key`.
